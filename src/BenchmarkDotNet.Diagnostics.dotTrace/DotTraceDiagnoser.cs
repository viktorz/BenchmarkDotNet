...
namespace BenchmarkDotNet.Diagnostics.dotTrace
{
    public class DotTraceDiagnoser : IProfiler
    {
        ...
        private string lastSnapshotFilePath;
        ...
        public void Handle(HostSignal signal, DiagnoserActionParameters parameters)
        {
            ...
            switch (signal)
            {
                ...
                case HostSignal.BeforeActualRun:
                    lastSnapshotFilePath = tool.Start(parameters);
                    break;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            if (lastSnapshotFilePath != null)
            {
                string currentDirectory = Directory.GetCurrentDirectory();
                string snapshotDirectory = Path.GetDirectoryName(lastSnapshotFilePath);
                if (snapshotDirectory != null)
                    logger.WriteLineInfo($"dotTrace snapshots can be found in '{snapshotDirectory.GetBaseName(currentDirectory)}'");
            }
        }
    }
}